...
namespace P2::ecs
{
	struct P2_API ResourceInfo
	{
		TypeID type;
		bool isConst = false;
	};

	struct P2_API QueryInfo
	{
		std::vector<TypeID> types;
		bool isConst = false;
	};

	struct P2_API SystemInfo
	{
	...
	struct P2_API MainThread
	{};

	struct P2_API GraphNode
	{
		TypeID typeID{};
		const std::type_info* typeInfo{};
		std::function<void(World&)> systemAdapter;
